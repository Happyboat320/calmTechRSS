...
def fetch_articles(
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = [executor.submit(fetch_source, source, since_utc, until_utc) for source in sources]
...
def fetch_source(
    ...
    with httpx.Client(headers=headers, follow_redirects=True, timeout=20.0) as client:
        try:
            ...
            return parse_feed(response.text, source, since_utc, until_utc)
...
def parse_feed(
    ...
    for entry in parsed.entries:
        ...
        if published_at < since_utc:
            continue
        if until_utc is not None and published_at > until_utc:
            continue
