...
- 事件重写：只对选中的新增事件类调用 LLM，每个请求包含该事件内文章的标题、摘要、正文片段和来源链接。
...
每次运行时，程序只处理候选窗口内尚未归属事件的文章。新文章会先和已有事件类的向量中心比较；相似度足够高时归入已有类，并更新该类的文章集合和向量中心。无法归入已有类的文章，才会在剩余集合中重新聚类并创建新事件类。本期简报只从本次新建类里按文章数量选择最多 5 条。
