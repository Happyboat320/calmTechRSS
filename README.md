...
`OPENAI_API_KEY` 和 `SITE_BASE_URL` 用途不同：前者只用于 `config/api.example.yml` 中的 API endpoint，后者只用于生成网页、RSS、原文归档和聚类日志链接。不要把 API endpoint 写进 `SITE_BASE_URL`。`SITE_BASE_URL` 建议写成带 `https://` 的完整站点根地址；程序会为漏写协议的值补 `https://`，避免浏览器把链接当成相对路径。
